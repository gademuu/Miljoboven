...
public class UpdateCaseView extends JPanel implements UpdateCaseViewSetter {
	...
	private JLabel l_crimeDate = new JLabel("Datum fˆr brott");
	...
	JComboBox c_unit = new JComboBox(
			new String[] { Unit.AVFALL, Unit.AVLOPPSHANTERING, Unit.IT,
					Unit.KLIMAT, Unit.MILJO, Unit.NATUR });
	...
	JComboBox c_status = new JComboBox(new String[] { Status.INRAPPORTERAD,
			Status.PABORJAD, Status.KLAR });

	private JLabel l_assign = new JLabel("Tilldela handl‰ggare");
	JComboBox c_assign = new JComboBox();
	...
	public void initialize() {
		...
		setSize(600, 600);
		Border outline = BorderFactory.createLineBorder(Color.black); // creating
																		// a
																		// border
																		// to
																		// highlight
		...
		crimeDateTextBoxPanel.setBorder(outline); // To show the component
													// border
		...
		crimeTypeTextBoxPanel.setBorder(outline); // To show the component
													// border
		...
		dateOfReportTextBoxPanel.setBorder(outline); // To show the component
														// border
		...
		seeFilesComboBoxPanel.setBorder(outline); // To show the component
													// border
	...
	public void dependingOnRole() {
		...
		case Role.ENHETSCHEF:
			...
			assignTextBoxPanel.add(c_assign);
			c.gridx = 2;
			c.gridy = 3;
			c.anchor = GridBagConstraints.WEST;
			// c.fill = GridBagConstraints.BOTH;
		...
		case Role.HANDLAGGARE:
			...
			/*
			 * //Upload files JPanel uploadFilesTextBoxPanel = new JPanel(new
			 * FlowLayout()); uploadFilesTextBoxPanel.add(l_uploadFiles);
			 * uploadFilesTextBoxPanel.add(tf_uploadFiles); c.gridx = 2; c.gridy
			 * = 3; c.anchor = GridBagConstraints.WEST; //c.fill =
			 * GridBagConstraints.BOTH; add( uploadFilesTextBoxPanel, c);
			 */
	...
	public void showCase(String id, Date crimeDate, String unit,
		...
		c_assign.removeAllItems();
		if (handlaggare != null) {
			for (int i = 0; i < handlaggare.length; i++) {
				c_assign.addItem(handlaggare[i]);

			}
		} else {
			c_assign.addItem("");
		}
		c_assign.setSelectedItem(assign);
